...
describe("TaskChatSystemNotice (PAP-443)", () => {
  ...
  it("moves Try again into the expanded notice footer", () => {
    ...
    const details = container.querySelector('[data-testid="task-chat-system-notice-details"]');
    expect(details?.querySelector('[data-testid="task-chat-no-live-path-try-again"]')).not.toBeNull();
    expect(container.querySelectorAll('[data-testid="task-chat-no-live-path-try-again"]')).toHaveLength(1);
  });

  it("shows the pending state and does not add Try again to unrelated notices", () => {
    renderNotice(
      {
        text: "Paperclip retried continuation, but it still has no live execution path.",
        presentation: {

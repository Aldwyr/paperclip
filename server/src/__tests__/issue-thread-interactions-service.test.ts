...
describeEmbeddedPostgres("issueThreadInteractionService", () => {
  ...
  it("keeps a capped review decision pending until the responsible user resolves it", async () => {
    ...
    expect((await interactionsSvc.listForIssue(issueId)).find(
      (interaction) => interaction.id === escalation.id,
    )?.status).toBe("pending");
  ...
  it("rejects public creation of server-owned capped-review metadata", async () => {
    ...
    }, { userId: "local-board" })).rejects.toThrow("payload.reviewEscalation is server-owned metadata");
  ...
  it.each([
    ...
    expect(resolved.interaction).toMatchObject({
      ...
      result: outcome === "changes_requested"
        ? { outcome: "rejected", reason, rejectionDisposition: "changes_requested" }
        : { outcome: "accepted" },
    });

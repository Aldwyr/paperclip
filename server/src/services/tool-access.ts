...
export function normalizeConnectionMethodConfig(
  ...
  const renderedServerUrl = method.defaults?.serverUrlTemplate
    ? method.defaults.serverUrlTemplate.replace(/\{([a-zA-Z0-9_-]+)\}/g, (_placeholder, key: string) => {
        const value = values[key];
        if (typeof value !== "string" || !value) {
          throw badRequest(`Missing connection setting for server URL: ${key}`);
        }
        return encodeURIComponent(value);
      })
    : method.defaults?.serverUrl;
  let endpoint: URL | null = null;
  if (renderedServerUrl) {
    try {
      endpoint = new URL(renderedServerUrl);
    } catch {
      throw badRequest("Connection server URL is invalid");
    }
    if (endpoint.protocol !== "https:") throw badRequest("Connection server URL must use HTTPS");
  }
...
export function toolAccessService(db: Db, options: ToolAccessServiceOptions = {}) {
  ...
  async function resolveCredentialHeaders(
    ...
    for (const ref of connection.credentialRefs) {
      let value: string;
      try {
        const configPath = `credentials.${ref.name}`;

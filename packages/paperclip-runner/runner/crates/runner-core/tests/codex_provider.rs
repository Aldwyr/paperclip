...
fn initialize_exit_reports_process_state_stage_and_redacted_stderr() {
    let result = CodexProvider::start(
        &LocalProviderConfig {
            kind: ProviderKind::Codex,
            command: PathBuf::from(env!("CARGO_BIN_EXE_fake-codex-app-server")),
            args: vec!["--exit-before-initialize".to_owned()],
            cwd: "/tmp".to_owned(),
            model: None,
            approval_policy: "never".to_owned(),
            instructions: "Do not start.".to_owned(),
    ...
    assert!(error.contains("provider=codex"), "{error}");
    assert!(error.contains("stage=initialize"), "{error}");
    assert!(error.contains("[REDACTED]"), "{error}");
    assert!(!error.contains("super-secret-provider-token"), "{error}");
}

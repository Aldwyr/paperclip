...
export interface LiveMatrixAugment {
  trace: EvalTraceEvidence;
  authorization: EvalAuthorizationExpectation;
  efficiency?: EvalEfficiencyEvidence;
  budget?: EvalEfficiencyBudget;
...
/**
 * Builds a scorable observation from a real-Codex live-matrix result. The live
 * matrix proves actual model tool choice and resulting control-plane state; the
 * augment supplies the trace, authorization, and efficiency evidence the matrix
 * result does not itself carry.
 */
export function observationFromLiveMatrix(
  result: CapabilityLiveCodexMatrixResult,
  augment: LiveMatrixAugment,
): EvalObservation {
  return {
    ...
    authorization: augment.authorization,
    trace: augment.trace,
 ...
 * operation counts as none. Use {@link observationFromLiveMatrix} for the
 * real-model path — this mapper scores the deterministic fake-agent surface.
 */

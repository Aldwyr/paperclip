import type { CapabilityJsonValue, CapabilityFixtureState } from "../mock-core/capability-control-plane-types.js";
...
const SECRET = /(bearer\s+[a-z0-9._-]+|(?:^|[^a-z0-9])(?:sk|pcp)_[a-z0-9._-]{8,}|api[_-]?key\s*[:=]|PAPERCLIP_API_KEY|OPENAI_API_KEY)/i;
const SECRET_KEY = /(^|[_-])(authorization|api[_-]?key|token|secret|password)($|[_-])|^(apiKey|accessToken|refreshToken)$/i;
...
function safe(value: unknown, key = ""): CapabilityJsonValue {
  ...
  return Object.fromEntries(Object.entries(value).map(([entryKey, entry]) => [entryKey, safe(entry, entryKey)]));
}
...
/** Field-by-field browser projection. Provider payloads and checkpoints never cross this boundary. */
function publicCompanyState(serialized: string): CapabilityJsonValue {
  ...
  return safe({
    ...
    artifacts: state.artifacts.map(({ contentRef: _contentRef, ...artifact }) => ({
      ...artifact,
      contentRef: "[withheld]",
    })),
    ...
    idempotency: state.idempotency.map(({ inputCanonical: _inputCanonical, ...record }) => record),
    faults: state.faults,
  });
}
...
export function projectCapabilityDevtools(snapshot: CapabilityLiveSessionSnapshot): CapabilityDevtoolsSnapshot {
  ...
  const history = retained[0]?.revision === seed.revision
    ? retained
    : [{

...
-- Some databases already contain native status decisions from the pre-merge
-- runner rollout, whose schema derived the run through work_assessments. Preserve
-- those rows by materializing the same authoritative relationship before the
-- direct run_id column becomes mandatory.
UPDATE "status_decisions" AS "decision"
SET "run_id" = "assessment"."run_id"
FROM "work_assessments" AS "assessment"
WHERE "decision"."run_id" IS NULL
	AND "assessment"."id" = "decision"."assessment_id"
	AND "assessment"."company_id" = "decision"."company_id"
	AND "assessment"."issue_id" = "decision"."issue_id";--> statement-breakpoint
DO $$
BEGIN
	IF EXISTS (SELECT 1 FROM "status_decisions" WHERE "run_id" IS NULL) THEN
		RAISE EXCEPTION 'Cannot derive status_decisions.run_id from the owning work assessment';
	END IF;
END
$$;--> statement-breakpoint
ALTER TABLE "status_decisions" ALTER COLUMN "run_id" SET NOT NULL;--> statement-breakpoint
-- The pre-merge runner rollout stored issue and contract ownership on each
-- native result but did not materialize it on heartbeat_runs. Validate that
-- every legacy run has one unambiguous owner, then populate the new direct
-- columns before installing the composite ownership foreign key.
DO $$
BEGIN
	IF EXISTS (
		SELECT 1
		FROM "native_run_results" AS "result"
		LEFT JOIN "heartbeat_runs" AS "run"
			ON "run"."company_id" = "result"."company_id"
			AND "run"."id" = "result"."run_id"
		WHERE "run"."id" IS NULL
	) THEN
		RAISE EXCEPTION 'Cannot derive heartbeat run ownership for an orphaned native result';
	END IF;

	IF EXISTS (
		SELECT 1
		FROM "native_run_results"
		GROUP BY "company_id", "run_id"
		HAVING count(DISTINCT ROW("issue_id", "completion_contract_id")) > 1
	) THEN
		RAISE EXCEPTION 'Cannot derive one heartbeat run owner from conflicting native results';
	END IF;

	IF EXISTS (
		SELECT 1
		FROM "native_run_results" AS "result"
		JOIN "heartbeat_runs" AS "run"
			ON "run"."company_id" = "result"."company_id"
			AND "run"."id" = "result"."run_id"
		WHERE ("run"."native_issue_id" IS NOT NULL AND "run"."native_issue_id" <> "result"."issue_id")
			OR ("run"."completion_contract_id" IS NOT NULL AND "run"."completion_contract_id" <> "result"."completion_contract_id")
	) THEN
		RAISE EXCEPTION 'Existing heartbeat run ownership conflicts with its native results';
	END IF;
END
$$;--> statement-breakpoint
WITH "legacy_run_ownership" AS (
	SELECT DISTINCT ON ("company_id", "run_id")
		"company_id", "run_id", "issue_id", "completion_contract_id"
	FROM "native_run_results"
	ORDER BY "company_id", "run_id", "id"
)
UPDATE "heartbeat_runs" AS "run"
SET
	"native_issue_id" = COALESCE("run"."native_issue_id", "ownership"."issue_id"),
	"completion_contract_id" = COALESCE("run"."completion_contract_id", "ownership"."completion_contract_id")
FROM "legacy_run_ownership" AS "ownership"
WHERE "run"."company_id" = "ownership"."company_id"
	AND "run"."id" = "ownership"."run_id"
	AND ("run"."native_issue_id" IS NULL OR "run"."completion_contract_id" IS NULL);--> statement-breakpoint
-- Terminal failures can have a finalization ledger row before a native result
-- exists. Derive issue ownership from that ledger as a second authoritative
-- source so its run-level foreign key can be installed without discarding the
-- failure record.
DO $$
BEGIN
	IF EXISTS (
		SELECT 1
		FROM "native_run_finalizations" AS "finalization"
		LEFT JOIN "heartbeat_runs" AS "run"
			ON "run"."company_id" = "finalization"."company_id"
			AND "run"."id" = "finalization"."run_id"
		WHERE "run"."id" IS NULL
	) THEN
		RAISE EXCEPTION 'Cannot derive heartbeat run ownership for an orphaned native finalization';
	END IF;
	...
	IF EXISTS (
		...
		RAISE EXCEPTION 'Cannot derive one heartbeat run issue from conflicting native finalizations';
	END IF;
	...
	IF EXISTS (
		...
		JOIN "heartbeat_runs" AS "run"
			ON "run"."company_id" = "finalization"."company_id"
			AND "run"."id" = "finalization"."run_id"
		WHERE "run"."native_issue_id" IS NOT NULL
			AND "run"."native_issue_id" <> "finalization"."issue_id"
	) THEN
		RAISE EXCEPTION 'Existing heartbeat run issue conflicts with its native finalization';
	END IF;
END
$$;--> statement-breakpoint
WITH "legacy_finalization_ownership" AS (
	SELECT DISTINCT ON ("company_id", "run_id")
		"company_id", "run_id", "issue_id"
	FROM "native_run_finalizations"
	ORDER BY "company_id", "run_id"
)
UPDATE "heartbeat_runs" AS "run"
SET "native_issue_id" = "ownership"."issue_id"
FROM "legacy_finalization_ownership" AS "ownership"
WHERE "run"."company_id" = "ownership"."company_id"
	AND "run"."id" = "ownership"."run_id"
	AND "run"."native_issue_id" IS NULL;--> statement-breakpoint
...
DO $$ BEGIN
	IF EXISTS (
		SELECT 1
		FROM "work_assessments" AS "assessment"
		LEFT JOIN "work_assessments" AS "superseded"
			ON "superseded"."company_id" = "assessment"."company_id"
			AND "superseded"."issue_id" = "assessment"."issue_id"
			AND "superseded"."run_id" = "assessment"."run_id"
			AND "superseded"."id" = "assessment"."supersedes_assessment_id"
		WHERE "assessment"."supersedes_assessment_id" IS NOT NULL
			AND "superseded"."id" IS NULL
	) THEN
		-- Preserve cross-run links created by the pre-merge rollout. NOT VALID
		-- keeps those historical rows while enforcing same-run ownership for all
		-- new or updated assessments.
		ALTER TABLE "work_assessments" ADD CONSTRAINT "work_assessments_supersedes_owner_fk" FOREIGN KEY ("company_id","issue_id","run_id","supersedes_assessment_id") REFERENCES "public"."work_assessments"("company_id","issue_id","run_id","id") ON DELETE no action ON UPDATE no action NOT VALID;
	ELSE

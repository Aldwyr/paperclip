...
describeEmbeddedPostgres("applyPendingMigrations", () => {
  ...
  it("backfills legacy native ownership and safely replays renumbered migrations", async () => {
    const connectionString = await createTempDatabase();
    await applyPendingMigrations(connectionString);

    const nativePersistenceHash = await migrationHash(
      "0227_modern_pandemic.sql",
    );
    const addresseeMigrationHash = await migrationHash("0229_last_anthem.sql");
    const companyId = "12000000-0000-4000-8000-000000000227";
    const agentId = "22000000-0000-4000-8000-000000000227";
    const runId = "32000000-0000-4000-8000-000000000227";
    const priorRunId = "33000000-0000-4000-8000-000000000227";
    const issueId = "42000000-0000-4000-8000-000000000227";
    const contractId = "52000000-0000-4000-8000-000000000227";
    const resultId = "62000000-0000-4000-8000-000000000227";
    const priorResultId = "63000000-0000-4000-8000-000000000227";
    const assessmentId = "72000000-0000-4000-8000-000000000227";
    const priorAssessmentId = "73000000-0000-4000-8000-000000000227";
    const decisionId = "82000000-0000-4000-8000-000000000227";
    const sql = postgres(connectionString, { max: 1, onnotice: () => {} });

    try {
      await sql.unsafe(`
          ALTER TABLE work_assessments
            DROP CONSTRAINT work_assessments_supersedes_owner_fk
        `);
      await sql`
          INSERT INTO companies (id, name, issue_prefix)
          VALUES (${companyId}, 'Legacy status decision fixture', 'LSD')
        `;
      await sql`
          INSERT INTO agents (id, company_id, name)
          VALUES (${agentId}, ${companyId}, 'Legacy decision agent')
        `;
      await sql`
          INSERT INTO issues (id, company_id, title, status)
          VALUES (${issueId}, ${companyId}, 'Legacy decision issue', 'in_progress')
        `;
      await sql`
          INSERT INTO completion_contracts (
            ...
            'legacy-contract-sha', 'system', 'migration-test'
          )
        `;
      await sql`
          INSERT INTO heartbeat_runs (
            ...
            (${runId}, ${companyId}, ${agentId}, 'succeeded', ${issueId}, ${contractId}),
            (${priorRunId}, ${companyId}, ${agentId}, 'succeeded', ${issueId}, ${contractId})
        `;
      await sql`
          INSERT INTO native_run_results (
            ...
            (
              ...
              'legacy-result-fingerprint', 'valid', ${JSON.stringify({ summary: "done" })}::jsonb,
              'legacy-result-sha'
            ),
            (
              ${priorResultId}, ${companyId}, ${issueId}, ${priorRunId}, ${contractId},
              'prior-result-fingerprint', 'valid', ${JSON.stringify({ summary: "retry" })}::jsonb,
              'prior-result-sha'
            )
        `;
      await sql`
          INSERT INTO work_assessments (
            ...
            ${priorAssessmentId}, ${companyId}, ${issueId}, ${priorRunId},
            ${contractId}, ${priorResultId}, 'run_terminal', ${companyId},
            'in_progress', 0, 'policy-v1',
            ${JSON.stringify({ disposition: "retry" })}::jsonb, 'prior-assessment-input-sha'
          )
        `;
      await sql`
          INSERT INTO work_assessments (
            ...
            ${JSON.stringify({ disposition: "done" })}::jsonb, 'legacy-assessment-input-sha',
            ${priorAssessmentId}
          )
        `;
      await sql`
          INSERT INTO status_decisions (
            ...
            ${JSON.stringify({ toStatus: "done" })}::jsonb, 'legacy-decision-sha'
          )
        `;

      await sql.unsafe(`
          ALTER TABLE native_run_results
            DROP CONSTRAINT native_run_results_run_contract_owner_fk;
          ALTER TABLE work_assessments
            DROP CONSTRAINT work_assessments_run_owner_fk;
          UPDATE heartbeat_runs
          SET native_issue_id = NULL, completion_contract_id = NULL
          WHERE id IN ('${runId}', '${priorRunId}');
          ALTER TABLE status_decisions DROP COLUMN run_id CASCADE;
        `);
      await sql`DELETE FROM "drizzle"."__drizzle_migrations" WHERE "hash" = ${nativePersistenceHash}`;
      await sql`DELETE FROM "drizzle"."__drizzle_migrations" WHERE "hash" = ${addresseeMigrationHash}`;
    } finally {
      await sql.end();
    }
    ...
    const verifySql = postgres(connectionString, {
      max: 1,
      onnotice: () => {},
    });
    try {
      const rows = await verifySql.unsafe<
        {
          ...
          supersedes_constraint_validated: boolean;
        }[]
      >(`
          SELECT
            decision.run_id,
            ...
            run.native_issue_id,
            run.completion_contract_id,
            assessment.supersedes_assessment_id,
            ownership_constraint.convalidated AS supersedes_constraint_validated
          FROM status_decisions decision
          JOIN heartbeat_runs run ON run.id = decision.run_id
          JOIN work_assessments assessment ON assessment.id = decision.assessment_id
          JOIN information_schema.columns column_info
            ON column_info.table_schema = 'public'
            AND column_info.table_name = 'status_decisions'
            AND column_info.column_name = 'run_id'
          JOIN pg_constraint ownership_constraint
            ON ownership_constraint.conrelid = 'work_assessments'::regclass
            AND ownership_constraint.conname = 'work_assessments_supersedes_owner_fk'
      ...
      expect(rows).toEqual([
        {
          run_id: runId,
          is_nullable: "NO",
          native_issue_id: issueId,
          completion_contract_id: contractId,
          supersedes_assessment_id: priorAssessmentId,
          supersedes_constraint_validated: false,
        },
      ]);
    } finally {
      await verifySql.end();
